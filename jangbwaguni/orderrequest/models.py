...
class Customer(models.Model):
    sig_date_cus = models.DateTimeField('date signup customer', auto_now_add=True) # 가입일 / 자동기입
    id_unique = models.CharField(unique=True, max_length=20) # 아이디 / 최대 20자리 / 중복 금지
    password = models.CharField(max_length=200) # 비밀번호 / 최대 25자리 / 암호화 했을 때 길이 길어지므로 200으로 설정
    nickname = models.CharField(max_length=10) # 닉네임 / 최대 10자리 / 중복 금지
    phone_num = PhoneNumberField(unique=True, null=False, blank=False) # 휴대폰 번호
